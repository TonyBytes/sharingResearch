...
app.controller('MyControllerDatabase', ['$scope', '$http', '$sce', function ($scope, $http, $sce) {
    ...
    $scope.updateVotes= function (post){
        var jsonData = angular.toJson({
            data: post
        });

        $http({
            url: "index.php/updateVotes",
            method: "POST",//security issues
    ...
    $scope.getComments= function (post){
        ...
        $http({
            url: "index.php/getComments",
            method: "POST",//security issues
            data:  "data=" + window.btoa(encodeURIComponent(jsonData)),
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        }).success(function (data, status, headers, config) {
            $scope.comments= data;
            $scope.postToLink= post.id;
        });
    }
    ...
    $scope.commentSubmit = function (newComment){
        ...
        var jsonData = angular.toJson({
            content: newComment,
            linkId: $scope.postToLink,
            poster:'Chenghao Lu'
        });
        $http({
            url: "index.php/newComment",
            method: "POST",//security issues
            data:  "data=" + window.btoa(encodeURIComponent(jsonData)),
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        }).success(function (data, status, headers, config) {
